...
| # | Defect | Status |
|---|---|---|
| 1 | squeezelite's systemd restart-rate limit exhausted by legitimate arbitration activity | **Still open, 2026-09-11** — a same-day attempt (`stop_unit`/`restart_after_release`) passed 35 scripted rounds clean, then recurred under live use with Bluetooth churn and was reverted; back to the original burst-limit-raise mitigation (5→20) — see this section's own addendum |
...
**Fixed, 2026-09-11 (same day, second session): George's decision was to fix the race, not raise the limit again.** The actual mechanism, found by tracing what was happening *between* the arbitration ladder's own checkpoints, not just at them: SIGKILL (the 2026-09-07/08 fix for squeezelite exiting cleanly on SIGTERM and never coming back) makes `Restart=on-failure` fire correctly, but that automatic restart then runs on its own fixed `RestartSec=2` cadence, completely independent of the arbitration ladder's own timing, testing whether it can open the ALSA device on every attempt - for as long as the device stays legitimately busy, it keeps failing and keeps restarting, burning through the burst limit on a clock of its own that nothing in `gexis-core` was watching or controlling. Fixed in `LmsAdapter`/`adapters/base.py`/`arbitration.py`: `signal_stop` now calls `stop_unit` (`systemctl stop`) instead of a raw kill signal - systemd does not auto-restart a unit that was asked to stop, so the independent background retry loop is gone entirely - and a new `restart_after_release` adapter hook brings squeezelite back explicitly, once, as the last step of `Supervisor.acquire()`, after the incoming renderer's own retry chance and volume restore. Full detail, including the residual risk this doesn't fully eliminate, in ADR-0010's matching amendment. Unit-tested (63 tests total).

**Verified on hardware, same session.** Confirmed directly that `systemctl stop` suppresses automatic restart entirely (9 seconds watched, no restart), and separately reproduced the named residual risk on purpose (a manual `systemctl start` against a still-busy device does trigger `Restart=on-failure` and climb `NRestarts`, exactly as documented, not just a theoretical caveat). Through the real `Supervisor.acquire()` path: 15 consecutive LMS-to-Spotify rounds and 20 consecutive Spotify-to-LMS rounds, zero restart-storm recurrences, all five units healthy throughout. Criterion 8's same-rate LMS↔Spotify distribution collected cleanly on both legs as a direct result - see Finding 015.

**Reverted, same day (second session): the named residual risk recurred for real, under live use rather than scripted rounds.** George continued using the reflashed image normally (LMS/Spotify switching, Bluetooth connecting and reconnecting several times in quick succession) and squeezelite failed into the identical restart-storm state again - `Start request repeated too quickly`, restart counter tripped the 20-burst limit - about two hours into the session, well after the 35 clean scripted rounds above. Consistent with rapid Bluetooth reconnect churn repeatedly re-triggering LMS's kill-and-explicit-restart cycle, with the explicit restart itself losing the race against a still-busy device often enough, across enough repetitions, to exhaust the burst limit regardless - exactly the residual risk this fix's own documentation named as "not proven impossible, only made meaningfully rarer." Rarer wasn't the same as fixed. Recovered manually again (`systemctl reset-failed && systemctl start`, after freeing whatever held the device first).

**Reverted:** `LmsAdapter.signal_stop` back to raw `kill_unit(force=True)`, `restart_after_release` removed (back to the inherited no-op) - `Restart=on-failure` is squeezelite's only path back again, the same mechanism this project ran on for weeks before today's attempt. This restores the *original* defect's own exposure (a rapid, uncontrolled restart-storm is possible again under heavy churn) rather than today's not-fully-eliminated replacement - a known, already-mitigated (burst raised 5→20) risk is preferable to an unproven one while George is actively depending on the system. **Not fixed for real, still open** - see the standalone Phase 2c issues overview for what a next attempt should account for that this one didn't (real, sustained multi-hour usage with Bluetooth churn, not just a scripted batch).
