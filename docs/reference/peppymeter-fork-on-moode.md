# Reference — PeppyMeter fork (meters + spectrum in one process) on moOde

**Provenance:** George's own earlier investigation, on moOde, against a
PeppyMeter fork that drives PeppyMeter and PeppySpectrum in one process.
Supplied 2026-09-16. **Not our measurement**: nothing here has been observed
on `gexis`, and the paths are moOde's. It is kept verbatim because the
failure modes it names are properties of that fork, not of moOde, and Phase 5
will meet them.

**What it bears on:** [ADR-0026](../decisions/0026-peppymeter-native-process-integration.md)
(native process, compositor-mediated), [ADR-0015](../decisions/0015-skin-renderer-peppymeter-format.md)
(skin format), and Phase 5 criteria 3, 4 and 5. See "What this changes for
us" at the end — added by Claude, clearly separated from George's text.
...
## What this changes for us (Claude, 2026-09-16)

Not George's text. Claims below are read off the document above; none has
been observed on `gexis`.

1. **Criterion 5 may need no renderer of ours.** "Skin rotates per track" is
   `meter = random` plus `random.change.title = True` in the fork's own
   config. Measured there at 0.6–1.5 s after the title change, which sits
   badly beside criterion 5's "next track's skin composited ahead of time"
   and criterion 4's "no visible construction". Either we accept the fork's
   timing or we pre-composite ourselves — a decision Phase 5 owes, not an
   implementation detail.
2. **Pairing is by name.** `spectrum.name` matches a section name, never an
   index. Criterion 3 already says so; this is independent confirmation, and
   the document records that assuming by-index cost real time.
3. **The cwd trap is ours too.** The fork `chdir`s into its spectrum
   directory, so every relative path afterwards resolves in the wrong place,
   and the spectrum config loader calls `os._exit(0)` when it finds nothing
   — a silent exit 0 with an empty log. Our unit must run `python3 -u`, set
   `base.folder` to an absolute path, and place the spectrum config where the
   process will look rather than where it is installed.
4. **Never infer "running" from the screen.** A dead meter process leaves its
   last frame on screen. Our own health check must read process state, and
   `pgrep -c -f` was unreliable for this process.
5. **`configparser` fails hard on a duplicate key.** Any config our image
   stage writes must replace in place, never append blindly.
6. **moOde's launcher fragility does not apply.** We build the image, so
   nothing overwrites our units. The transferable part is the rule: own the
   file you depend on.
