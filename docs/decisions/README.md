...
| [0055](0055-which-output-the-device-plays-to.md) | Which output the device plays to | **Proposed** — four playback outputs measured on the device and **two of them have no volume control at all**, so choosing one *is* [0046](0046-fixed-output-hides-the-slider.md)'s fixed output. The switch is two lines of `output.conf` because everything already goes through [0009](0009-alsa-device-indirection.md)'s `pcm.output`, and the visualiser follows for free. Open: which outputs to offer, given that one HDMI is the panel's own connector and the other is empty. Phase 9 **9j** |
