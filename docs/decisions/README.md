...
| [0052](0052-the-volume-path.md) | The volume path: what moves the level, how fast, and how loud it may get on its own | **Accepted** — George, 2026-09-22. The boot number is not the hazard, an unattended jump is, so the *restore* gets a ceiling (−20 dB) and the boot level stays at silence. Direct libasound writes (5.7 ms against 16.4) and a ramp to each new target, because a drag only ever delivers a sample of itself. `travel_curve` renamed to what it does; `max_ceiling` enforced in one place; the mirror rate-limited; Spotify stops attenuating the stream. On [Finding 045](../findings/045-the-volume-path-measured.md). Phase 9 **9i** |
