...
| Volume curve | [R][H] | **Recorded as a setting 2026-09-23 on George's instruction** (*"record the linear and cubic curves as settings for the next step"*), **still hardcoded**. [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §3: one curve for every renderer and for the panel, over 60 dB, with zero as silence. **Cubic** ships — half travel −15.5 dB, the shape a volume control usually has — and **Linear (dB)** is the even-decibels alternative it replaced, where half travel was −30 dB and George found the bottom half unusable. Was "Slider travel curve" offering `Perceptual`/`dB-linear`, which described a window rather than a curve |
