...
| Setting | Mark | Notes |
|---|---|---|
| Output | [R] | **New row, built 2026-09-23** ([ADR-0055](0055-which-output-the-device-plays-to.md)). Discovered from ALSA, not written down: the HAT's EEPROM is read at boot, so the card and its volume control's name are properties of whatever board is fitted. All four are offered and an unplugged one says *nothing connected*; an output with no volume control forces fixed output |
| Output mode: fixed / variable | [R] | **Wired and built 2026-09-23** — the last row in Audio, and the oldest unbuilt decision in the project. [ADR-0046](0046-fixed-output-hides-the-slider.md): the slider is *absent*, not disabled, both triggers carry a padlock, and the drawer still opens onto the sentence. Confirmation required (ADR-0044's `warn`), and it applies after stop rather than mid-track |
| Boot volume level | **[deleted]** | **George, 2026-09-23: "Go ahead and remove it then."** Wired that morning, gone the same afternoon with the unit that read it. Measured over two boots ([Finding 047](../findings/047-where-the-volume-actually-goes.md) §10): the converter comes up at −20 dB of its own accord, nothing carries a level across a boot, and nothing plays before a renderer acquires — at which point [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §5 sets the level from the renderer itself. [ADR-0018](0018-volume-and-output-modes.md) amended; the `alsa-restore` mask is the half that survives and stays |
| Maximum volume | [R] | **Decided 2026-09-22 by [ADR-0052](0052-the-volume-path.md) §3 as amended, and expressed as a percentage from 2026-09-23** (George: *"everything must be in percentage — if we say 80% then the max output can only be 80% of the max volume"*). `max_ceiling`, unset by default. It is the *top of every scale*, not a limit above one: 100% on the panel, 100 in LMS and 100 on a connected phone all mean the level the slider at 80 would make. Applied as a shift on every position-to-dB map, so every step keeps its size |
| Restore floor for a *remembered* level | **[out of scope]** | **George, 2026-09-23: "not needed".** Both were `[?]` — decisions owed rather than behaviour missing — and both are answered instead by [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §5: a renderer is asked where it is when it takes the device, so neither a floor under a remembered level nor a rule about when the boot default applies decides anything a renderer has not already said. The row is gone from the registry; `restore_volume_floor_db` stays hardcoded in `config.py` |
| Boot default mid-session, or only at true cold boot? | **[out of scope]** | **George, 2026-09-23: "not needed".** Both were `[?]` — decisions owed rather than behaviour missing — and both are answered instead by [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §5: a renderer is asked where it is when it takes the device, so neither a floor under a remembered level nor a rule about when the boot default applies decides anything a renderer has not already said. The row is gone from the registry; `restore_volume_floor_db` stays hardcoded in `config.py` |
| Per-renderer volume memory on/off | **[deleted]** | **George, 2026-09-23: "delete them".** Wired that morning and gone by the afternoon, with the machinery behind it. Since [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §5 a renderer is *asked* where it is when it takes the device, and the renderer's own memory is the real one — LMS keeps it per player, Spotify per device, a phone per device — so ours was a second, worse copy. **Measured before deleting: 12 acquisitions after §5 landed, 12 answers, 0 fallbacks.** `renderer_volume.py`, `volume.json` and `restore_volume_floor_db` went with it; George's decision of 2026-09-07 is retired, having been right for as long as renderers were silent about their volume |
| Which renderers are volume-managed | [R] | **Wired 2026-09-23**, readonly: it now reports what the adapters declare (`capabilities.volume_managed`) rather than a literal that could drift from them.<br> `capabilities.volume_managed`; Bluetooth excluded pending Finding 006 |
| Volume curve | [R] | **Wired 2026-09-23.**<br> **Recorded as a setting 2026-09-23 on George's instruction** (*"record the linear and cubic curves as settings for the next step"*), **still hardcoded**. [ADR-0054](0054-one-curve-and-the-renderers-own-number.md) §3: one curve for every renderer and for the panel, over 60 dB, with zero as silence. **Cubic** ships — half travel −15.5 dB, the shape a volume control usually has — and **Linear (dB)** is the even-decibels alternative it replaced, where half travel was −30 dB and George found the bottom half unusable. Was "Slider travel curve" offering `Perceptual`/`dB-linear`, which described a window rather than a curve |
...
| Transition screen threshold, and the exempt pairs | [R] | **Wired 2026-09-25, and the `?` this carried is answered** ([ADR-0078](0078-the-transition-screen-waits-for-the-threshold.md)). George found it by its orange dot: *"the transition screen threshold set at 1 (also inside this should be a selection bar with 0.5 increments up to 3s)."* The number now means **how long a takeover has to be in flight before the panel explains it** — a slider from 0 to 3 s in 0.5 s steps, where 0 is the behaviour before this. Nothing had ever read it: ADR-0010 set 1 s and the exempt list was compared against it once, by hand.<br> **The "evidence-gated rather than preference" worry is answered rather than overruled**: measurement decides how long a takeover took, the person decides how long a wait deserves an explanation. A delay needs no declaration at all, so it is more evidence-driven than a list, and it can tell that *this* takeover is slow, which a per-pair median cannot. The exempt pairs stay — measured fact, published as Phase 4 criterion 4 requires. Five runs on the panel, one per value ([Finding 069](../findings/069-the-transition-screen-against-the-threshold.md)) |
...
| Update artist portraits | [R] | **Appended and built 2026-09-24, asked for by George** ([ADR-0059](0059-artist-portraits-in-the-list.md)). An action: every album artist looked up on fanart.tv, LMS's picture kept where fanart has none. Checks all of them on every press, his call. Measured on his library: **917 of 917 in seven minutes, 525 portraits found**. **Moved into the LMS group, 2026-09-25** — George: *"Let's move the pictures from fanart group in enrichment to sources LMS. Without Lms there is no point in running enrichment of that type"*, then *"Not a subgroup, just take the 3 settings and put them together with enabling lms and server."* So they are plain rows beside Enabled and Server, not a section of their own, and the `Pictures from fanart.tv` heading is gone. The sweep walks the LMS library, so all three hide with `lms_enabled` ([ADR-0079](0079-with-lms-off-the-panel-is-two-screens.md)'s pattern). |
| Update album covers | [R] | **Appended and built 2026-09-24.** The same walk, for every album rather than only the ones with no cover - fanart returns an artist's albums in the artist call, so it needs no per-album request (Finding 054 §9). One of the two runs at a time. **Moved into the LMS group, 2026-09-25** — George: *"Let's move the pictures from fanart group in enrichment to sources LMS. Without Lms there is no point in running enrichment of that type"*, then *"Not a subgroup, just take the 3 settings and put them together with enabling lms and server."* So they are plain rows beside Enabled and Server, not a section of their own, and the `Pictures from fanart.tv` heading is gone. The sweep walks the LMS library, so all three hide with `lms_enabled` ([ADR-0079](0079-with-lms-off-the-panel-is-two-screens.md)'s pattern). |
| Progress | [R] | **Appended and built 2026-09-24**, readonly, in George's own words: *"X out of Y processed (searched for), Z artist portraits found."* The third number matters - fanart has nothing for 43% of his album artists, so a run that ends at Y of Y with 57% found has worked. **Moved into the LMS group, 2026-09-25** — George: *"Let's move the pictures from fanart group in enrichment to sources LMS. Without Lms there is no point in running enrichment of that type"*, then *"Not a subgroup, just take the 3 settings and put them together with enabling lms and server."* So they are plain rows beside Enabled and Server, not a section of their own, and the `Pictures from fanart.tv` heading is gone. The sweep walks the LMS library, so all three hide with `lms_enabled` ([ADR-0079](0079-with-lms-off-the-panel-is-two-screens.md)'s pattern). |
...
| Confidence threshold | [R] | ADR-0012: below it, show nothing. A confidently wrong artist biography is worse than a blank panel. **Wired and surfaced 2026-09-24** (ADR-0059), read on every ask rather than captured at startup, and it now gates the artwork sweep too: `Head` resolved at 100 and nothing can say it is the right Head. **It had carried `surfaced: false`** — inventoried and not offered — which George found the moment it started mattering: *"I am not seeing the confidence setting in the enrichment menu."* **And it was on the wrong scale**: 0-1 with a 0.05 step, against the 0-100 MusicBrainz scores and `CONFIDENCE_MIN` use, so any value would have collapsed to 0 or 1. Now 0-100%, default 90 - ~~beside the two buttons it governs~~, which it no longer is. **Stayed in Enrichment when the fanart rows left, 2026-09-25.** It sat in that sub-group because it also gates the sweep, but `EnrichmentService` applies it to **every provider's answer for every renderer** - it is what stops a Bluetooth track getting a confidently wrong biography. Under LMS it would have hidden with `lms_enabled` and taken that with it |
...
| Settings backup / restore | [R] | **Wired and surfaced 2026-09-25** ([ADR-0083](0083-a-backup-leaves-the-device.md)), George: *"Wire it for both actions - backup and restore."* It had been inventoried, unsurfaced and unwired since this record, and that cost a hand copy over SSH the day the card was reflashed - `settings.db` holds 41 rows including three API keys, and `enrichment.db` held 3,664 enrichment rows and 7,061 notes.<br> Two rows: `backup` writes an archive, `restore` is a `list` whose items are the archives found. **Into a share of its own**, because a backup that stays on the device does not survive the event it exists for; a browser download would reach a phone and not the panel. A restore replaces everything and **reboots**, and says so first |
