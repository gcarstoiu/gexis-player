...
| Boot volume level | [R] | **Wired 2026-09-23.** Read by `gexis_core.boot_volume`, its own unit ordered before every renderer, so it applies at the next boot — which is what a boot volume means. `core.toml`'s `boot_volume_steps` stays as the fallback, and every failure to read the row still writes the safe level.<br> ADR-0018; `boot_volume_steps`, raw 60 = −90 dB, confirmed 2026-09-06. Fixed safe level, never restored from the last session |
