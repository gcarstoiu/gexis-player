...
| Per-renderer volume memory on/off | [R] | **Wired 2026-09-23**, read on every remember and every restore so a change applies to the next acquisition. Off means every renderer starts from the boot level; what is already stored is kept, so turning it back on returns the device to where it was.<br> George's decision 2026-09-07, currently unconditional |
